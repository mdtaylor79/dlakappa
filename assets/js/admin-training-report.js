window.TrainingReport = (function () {
  let allRecords = [];
  let totalMembers = 0;
  let didInit = false;
  ...
  function init() {
    ...
    document.getElementById("searchInput-report").addEventListener("input", renderTable);
  ...
  async function loadData() {
    ...
    if (error) {
      document.getElementById("byTrainingWrap-report").innerHTML =
        `<div class="empty">Error loading records: ${escapeHtml(error.message)}</div>`;
      document.getElementById("tableWrap-report").innerHTML = "";
      return;
    }
  ...
  function renderByTraining() {
    const wrap = document.getElementById("byTrainingWrap-report");
  ...
  function getFilteredRecords() {
    const search = document.getElementById("searchInput-report").value.trim().toLowerCase();
  ...
  function renderTable() {
    ...
    const wrap = document.getElementById("tableWrap-report");
