  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (burger && menu) {
      ...
      document.addEventListener('click', e => {
        if (menu.classList.contains('open') && !menu.contains(e.target) && e.target !== burger && !burger.contains(e.target)) {
          menu.classList.remove('open');
        }
      });
      // Close when a non-dropdown leaf link is tapped on mobile
      menu.querySelectorAll('li:not(.has-drop) > a').forEach(a => {
        a.addEventListener('click', () => { if (window.innerWidth <= 900) menu.classList.remove('open'); });
      });
    }
